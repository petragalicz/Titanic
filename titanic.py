...
train_table[['Age', 'Survived']].groupby(
    ['Age'], as_index=False).mean().sort_values(by='Survived', ascending=False)

# Survived by fare in % -- fare takes on too many values to have a good overview
train_table[['Fare', 'Survived']].groupby(
    ['Fare'], as_index=False).mean().sort_values(by='Survived', ascending=False)
